...
{  
    ...
    std::cout << "[DemoListener] Initialized without TTS or playback." << std::endl;
...
void AIUI_Node::sendMessage(const std::string& message) {
    auto request = std::make_shared<deepseek_interfaces::srv::DeepseekChat::Request>();
    request->position_text = message;
    waiting_for_response_ = true;
    cout << "正在思索整理中..." << endl;
    // 使用回调处理响应，不保存 future
    client_->async_send_request(
        request,
        [this](rclcpp::Client<deepseek_interfaces::srv::DeepseekChat>::SharedFuture future) {
            this->response_callback(future);
        }
    );
}

void AIUI_Node::response_callback(rclcpp::Client<deepseek_interfaces::srv::DeepseekChat>::SharedFuture future) {
    try {
        auto response = future.get();
        if (response) {
            std::cout << "[Deepseek响应] 楼层: " << response->floor
                      << ", 房间号: " << response->room
                      << ", 成功标志: " << std::boolalpha  << response->success << std::endl;

            int success_ = response->success ? 1 : 0;
            int floor_ = response->floor;
            int room_ = response->room < 0 ? 0 : response->room; 
            std::string code = std::to_string(success_) + std::to_string(floor_) + std::to_string(room_);
            std_msgs::msg::String msg;
            msg.data = code;
            deepseek_response_pub_->publish(msg);

            is_wakeup_called = false;
            waiting_for_response_ = false;
        }
    } catch (const std::exception &e) {
        RCLCPP_ERROR(this->get_logger(), "处理响应时出错: %s", e.what());
    }
}
...
{
    std::cout << "awake_flag:" << static_cast<int>(msg->data) << std::endl;
    awake_flag = msg->data;
    if (!is_wakeup_called)
    {
        wakeup();
...
AIUI_Node::AIUI_Node(const std::string &node_name,
    ...
    RCLCPP_INFO(this->get_logger(),"%s node init success!\n",node_name.c_str());

    /***服务客户端创建***/
    client_ = this->create_client<deepseek_interfaces::srv::DeepseekChat>("extract_location");
    deepseek_response_pub_ = this->create_publisher<std_msgs::msg::String>("deepseek_response", 10);
    /***识别文本话题发布者创建***/
